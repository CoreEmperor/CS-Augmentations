...
public class InvisibleChemBlock extends Block {
    public InvisibleChemBlock(Properties pProperties) {
        super(pProperties);
    }
    ...
    public void playerWillDestroy(Level pLevel, BlockPos pPos, BlockState pState, Player pPlayer) {
        BlockPos chemPos = findChemBlock(pLevel, pPos);
        if (chemPos != null) {
            pLevel.removeBlock(chemPos, false);
        }
        super.playerWillDestroy(pLevel, pPos, pState, pPlayer);
    }

    private BlockPos findChemBlock(Level pLevel, BlockPos pPos) {
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            BlockPos adjacentPos = pPos.relative(direction);
            BlockState adjacentState = pLevel.getBlockState(adjacentPos);
            if (adjacentState.getBlock() instanceof ChemistryBlock) {
                return adjacentPos;
            }
        }
        return null;
